...
class GetData {
  ...
  getHomeStudioApi({required onSuccess, required onError}) async{
    ...
    if( response.statusCode == 200) {
      print(response.body);
      final serviceListModalClass = serviceListModalClassFromJson(response.body);
  ...
  getNotificationApi({required onSuccess, required onError}) async{
    ...
    var response = await http.get(Uri.parse(AppApi.getNotificationListApi),headers: {
    ...
    if( response.statusCode == 200) {
      print(response.body);
      final NotificationListModalClass = notificationModalClassFromJson(response.body);
